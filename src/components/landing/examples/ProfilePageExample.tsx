...
const ProfilePageExample: React.FC = () => {
  ...
  // Utility function to test if image URL is accessible
  ...
  const handleProfilePicChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.log('Uploaded photoURL:', url);
      
      // Test if the image URL is accessible
      console.log('Testing image URL accessibility...');
      const isImageAccessible = await testImageURL(url);
      console.log('Image URL accessible:', isImageAccessible);
      ...
      if (auth.currentUser) {
        try {
          ...
          console.log('Updated Firebase Auth profile with photoURL');
        } catch (authError) {
          console.warn('Failed to update Firebase Auth profile:', authError);
        }
      }
      
      // Wait for Firestore to update, then re-fetch
      console.log('Waiting for Firestore update...');
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      const updatedProfile = await userService.getUserProfile(userProfile.uid);
      console.log('Re-fetched profile photoURL:', updatedProfile?.photoURL);
      
      if (updatedProfile) {
        setUserProfile(updatedProfile);
        console.log('Profile updated successfully');
      } else {
        console.error('Failed to re-fetch updated profile');
        ...
        >
          <div className="flex flex-col lg:flex-row gap-6 items-center lg:items-start">
            ...
            <div className="relative group">
              ...
              >
                {(() => {
                  console.log('Rendering profile photoURL:', userProfile.photoURL);
                  return userProfile.photoURL ? (
                    <>
                      <img
                        ...
                        onError={(e) => {
                          console.error('Failed to load profile image:', userProfile.photoURL);
