...
const MainPage: React.FC = () => {
  ...
  const [announcements] = useState([
    { id: 1, title: t('announcement.newFeature'), content: t('announcement.dailyQuiz'), date: '2025-05-18', isNew: true },
    { id: 2, title: t('announcement.maintenance'), content: t('announcement.maintenanceDetail'), date: '2025-05-17', isNew: true },
    { id: 3, title: t('announcement.welcome'), content: t('announcement.welcomeDetail'), date: '2025-05-15', isNew: false },
  ]);
  ...
  useEffect(() => {
    const fetchChapters = async () => {
      try {
        const chapters = await chapterService.getAll(true);
    ...
    const fetchUserProgress = async () => {
      if (currentUser?.uid) {
        try {
          ...
          if (userProfile) {
            setCompletedChapters(userProfile.chaptersCompleted?.length || 0);
          }
        } catch (error) {
    ...
    return (
      <div className="fixed inset-0 flex justify-center items-center bg-gradient-to-br from-[#0F172A] via-[#1E293B] to-[#334155]">
        <div className={`${CardBg} ${CardBorder} ${CardShadow} ${CardRadius} p-8 text-center`}>
          <h2 className="text-3xl font-serif font-bold text-[${Gold}] mb-4">{t('loading')}</h2>
  ...
  if (!currentUser) {
    return (
      <div className="fixed inset-0 flex justify-center items-center bg-gradient-to-br from-[#0F172A] via-[#1E293B] to-[#334155]">
        <div className={`${CardBg} ${CardBorder} ${CardShadow} ${CardRadius} p-8 text-center`}>
          <h2 className="text-3xl font-serif font-bold text-[${Gold}] mb-4">{t('auth.required')}</h2>
          <p className="text-gray-300 mb-6">{t('auth.pleaseLogin')}</p>
          <button onClick={() => navigate('/login')} className={Button}>{t('login')}</button>
        </div>
      </div>
    );
  }
  ...
  const userName = userData?.displayName || currentUser?.displayName || currentUser.email?.split('@')[0] || t('user.default');
  const achievements = [
          ...
          <div className="flex justify-between items-center w-full h-full">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col justify-center">
                <span className="text-white text-base md:text-lg font-semibold leading-tight">{t('welcome', { name: userName })}</span>
                <span className="text-xs md:text-sm text-white/60 mt-0.5">{t('welcome.back')}</span>
              </div>
            </div>
            {/* Icons group - Announcements and Language Switcher */}
          ...
          <>
            ...
            <div className="bg-white/5 rounded-xl md:rounded-2xl p-4 md:p-6 shadow flex flex-col md:flex-row items-start md:items-center gap-4 mb-6 md:mb-8 border border-white/10 w-full">
              <div className="flex-1 w-full">
                <div className="text-base md:text-lg font-serif font-extrabold text-white mb-2">{t('your.progress')}</div>
                <div className="text-sm md:text-base text-white/70 mb-2">{t('completed.chapters', { completed: String(completedChapters), total: String(totalChapters) })}</div>
