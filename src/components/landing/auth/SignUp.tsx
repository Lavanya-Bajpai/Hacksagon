  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#f5e1a0] via-[#e2d8c0] to-[#bfa77a] py-6 md:py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      >
        ...
        >
          <QuillIcon />
          <h2 className="text-2xl md:text-3xl font-bold text-amber-800 font-serif">{t('signup')}</h2>
        </motion.div>
        <div>
          ...
          <p className="mt-2 text-center text-xs md:text-sm text-gray-700">
            ...
            <Link to="/login" className="font-medium text-accent hover:text-accent/80 underline">
              {t('sign_in_to_your_existing_account')}
            </Link>
          </p>
        </div>
        ...
        {signupSuccess ? (
          ...
          >
            ...
            <h3 className="font-bold text-base md:text-lg mb-2">{t('account_created_successfully')}</h3>
            <p className="text-sm md:text-base">{t('you_will_be_redirected_to_the_main_page_shortly')}</p>
          </motion.div>
        ) : (
