...
export class UserService {
  ...
  async getUserProfile(uid: string): Promise<UserProfile | null> {
    try {
      ...
      if (doc) {
        console.log('UserService: Profile photoURL:', doc.photoURL);
        console.log('UserService: Has photoURL:', !!doc.photoURL);
      }
      return doc;
    } catch (error) {
      console.error('UserService: Error fetching user profile:', error);
  ...
  async uploadProfilePicture(uid: string, file: File): Promise<string> {
    try {
      ...
      console.log('Updating user profile with photoURL...');
      await this.updateUserProfile(uid, { photoURL: url });
      console.log('User profile updated successfully');
      
      // Verify the update
      const updatedUser = await this.getUserProfile(uid);
      console.log('Verification - Updated user photoURL:', updatedUser?.photoURL);
