...
import { uploadChapters } from '../../utils/upload-chapters';
...
const AdminDashboard: React.FC = () => {
  ...
  const handleUploadChapters = async () => {
    ...
    try {
      ...
      setStats(prev => ({ ...prev, chapters: chapters.length }));
      alert('Successfully uploaded 22 fundamental rights chapters!');
    } catch (error) {
      console.error('Error uploading chapters:', error);
      alert('Error uploading chapters. Check console for details.');
    } finally {
      setUploading(false);
    }
  };
      ...
      <main className="flex-1 flex flex-col px-4 md:px-12 py-8">
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          ...
          <button 
            ...
            disabled={uploading}
            className="inline-flex items-center gap-2 bg-orange-500 text-white px-5 py-2 rounded-lg shadow hover:bg-orange-600 transition-colors font-semibold disabled:opacity-50"
          >
            <NavIcon path={statIcons.chapters} className="w-5 h-5" /> 
            {uploading ? 'Uploading...' : 'Upload 22 Chapters'}
          </button>
        </div>
